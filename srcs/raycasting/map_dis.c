...
static void draw_arrow_player(t_data *d, int x, int y, int color)
{
	...
	while (++dy < 10)
	{
		...
		while (++dx < 10)
		{
			pixel_x = x * 10 + dx + 22;
			pixel_y = y * 10 + dy + 22;
			if (d->player->orientation == 'W' && dx >= arrow_size)
				my_mlx_pixel_put(d, pixel_x, pixel_y, color);
			else if (d->player->orientation == 'E' && dx <= 10 - arrow_size)
				my_mlx_pixel_put(d, pixel_x, pixel_y, color);
			else if (d->player->orientation == 'S' && dy >= arrow_size)
				my_mlx_pixel_put(d, pixel_x, pixel_y, color);
			else if (d->player->orientation == 'N' && dy <= 10 - arrow_size)
				my_mlx_pixel_put(d, pixel_x, pixel_y, color);
			else
